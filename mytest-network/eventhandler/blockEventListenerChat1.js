...
//const apiUrl2 = 'http://localhost:3000/api/refreshComponent1?component=servicerequest';
const apiUrl3 = 'http://localhost:3000/api/notifyJoinRoom';
...
async function main() {
    try {
        ...
        const contract = await network.getContract(`chatcc`);
	    ...
	    console.log("test message before insert1");
            
            if(room_id){
                console.log("test message before insert");

                const query = `INSERT INTO public."chat"(room_id,room_name, "userId", "userName",message,"messageid","description","timestamp") VALUES ('${event.room_id}','${event.room_name}', '${event.userId}','${event.userName}','${event.message}','${event.messageid}','${event.description}',current_timestamp) ON CONFLICT ("room_id") DO UPDATE SET room_id=excluded.room_id, room_name=excluded.room_name, "userId"=excluded."userId", "userName"=excluded."userName","message"=excluded."message","messageid"=excluded."messageid","description"=excluded."description","timestamp"=current_timestamp`;
                ...
                const query2 = `INSERT INTO public."chat_history"(room_id,room_name, "userID", "message","messageid","description","timestamp") VALUES ('${event.room_id}', '${event.room_name}','${event.userId}','${event.message}','${event.messageid}','${event.description}',current_timestamp) ON CONFLICT("messageid") DO UPDATE SET room_id=excluded.room_id, room_name=excluded.room_name, "userID"=excluded."userID", message=excluded.message,messageid=excluded.messageid, description=excluded.description,"timestamp"=current_timestamp`;
            ...
            // Make a POST request
             if(message == "added to room"){
              
	        fetch(apiUrl3,{
                method: 'POST',
                body: `{"roomId":"${event.room_id}","eventid":"${event.roomid}","title":"New User Joined","user":"${event.userName}","message":"${event.userName} joined room ${event.room_name}","description":"${event.description}","messageid":"${event.messageid}", "eventype":"chat","eventid":"${event.room_id}","time":"${text1}"}`
      ...
      }else
        {
            fetch(apiUrl,{
                method: 'POST',
                body: `{"roomId":"${event.room_id}","eventid":"${event.room_id}","title":"new message in ${event.room_name}","user":"${event.userName}","message":"${event.message}","description":"${event.description}","messageid":"${event.messageid}", "eventype":"chat","time":"${text1}"}`
                  // body: '{\"message\":\"Request created from ${event.created_by} having request ID : ${event.requestId}\"}'
          ...
          // Make a POST request
            ...
            //fetch(apiUrl2,{
              //  method: 'POST',
            //    body: `{"roomId":"${event.room_id}","user":"${event.userId}","message":"${event.message}","description":"${event.description}","messageid":"${event.messageid}","eventype":"chat","eventid":"${event.room_id}", "time":"${text1}"}`
                  // body: '{\"message\":\"Request created from ${event.created_by} having request ID : ${event.requestId}\"}'
