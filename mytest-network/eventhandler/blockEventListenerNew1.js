...
`
               ...
               const queryextn = `INSERT INTO public."client_service_req_extn"("requestId",message,message_from,messageid,date) VALUES ('${event.requestId}','${event.message}','${event.message_from}','${event.messageid}',current_timestamp) ON CONFLICT ("messageid") DO UPDATE SET "requestId"=excluded."requestId",message=excluded.message,message_from=excluded.message_from,messageid=excluded.messageid,date=current_timestamp`;
              ...
              //}
                ...
                fetch(apiUrl,{
                      method: 'POST',
                      body: `{"user":"${createdby}","title":"${eventid} for ${requestid}","message":"${text}","component":"notificationBar","eventype":"serviceRequest","eventid":"${eventid}","time":"${text1}"}`
