...
configtxlator proto_decode --input ./channel-artifacts/config_block.pb --type common.Block | jq .data.data[0].payload.data.config > ./channel-artifacts/Org3MSPconfig.json
jq '.channel_group.groups.Application.groups.'Org3MSP'.values += {"AnchorPeers":{"mod_policy": "Admins","value":{"anchor_peers": [{"host": "'${HLF_ORG_PEER1_ID}'","port": '7051'}]},"version": "0"}}' ./channel-artifacts/Org3MSPconfig.json > ./channel-artifacts/Org3MSPmodified_config.json
configtxlator proto_encode --input "./channel-artifacts/Org3MSPconfig.json" --type common.Config >./channel-artifacts/original_config.pb
configtxlator proto_encode --input "./channel-artifacts/Org3MSPmodified_config.json" --type common.Config >./channel-artifacts/modified_config.pb
...
peer channel update -o centos81:7050 --ordererTLSHostnameOverride $HLF_ORDR_ID -c mytestchannel -f ./channel-artifacts/Org3MSPanchors.tx --tls --cafile $ORDERER_CA
